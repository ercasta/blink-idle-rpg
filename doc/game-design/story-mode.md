...
The narrative system produces a **textual log** of the party's journey. Messages are generated during story mode simulation whenever significant events occur. Each message has a **verbosity level**:

| Level | Name | Description | Example |
|-------|------|-------------|---------|
| 1 | **Headlines** | Major events only | "Day 5 — The party arrives at Ironhold." |
| 2 | **Standard** | Decisions, encounters, arrivals, hero preferences | "Nyx proposes to push toward Ember Hollow through dense woodland." / "The party encounters 3 Goblin Scouts!" |
| 3 | **Detailed** | Combat actions, effects, individual votes, terrain descriptions | "Roland uses Power Strike." / "The enemy staggers from the blow." / "The party defeats the Goblin Scouts. 3 enemies slain, 75 XP earned." |

The player can toggle verbosity in the UI during playback and on the results screen. All three levels are generated; the UI filters entries whose `level` ≤ the selected verbosity.

### Logging Architecture

Narrative logging is implemented only in **story mode** (not fight mode) to avoid making the game too heavy. The logging system operates at three verbosity levels as described above. The narrative log is ephemeral — it is available during playback and on the results screen but is **not** persisted in run history or IndexedDB.

**BRL Components** (`story-mode.brl`):
...
Messages are generated deterministically in `WasmSimEngine._generateNarrativeLog()` after the simulation completes. The generator uses:

- **Seed**: The run's random seed (converted to a 32-bit number) for deterministic hash-based decisions.
- **Hero names and traits**: Each hero's name, class, and personality traits (especially `rc` for cautious/risky, `od` for offensive/defensive) influence their proposals.
- **Game outcome**: Total encounters, locations visited, towns rested, ambushes survived, and whether the final destination was reached.
- **Environment settings**: The dominant element determines location name themes (fire-themed, water-themed, etc.).

The narrative log includes these message categories:

| Category | Level | Description |
|----------|-------|-------------|
| Journey start | 1–3 | Opening messages setting the scene |
| Day start | 1 | "Day N begins." |
| Hero proposals | 2 | Each hero's preference: "Nyx proposes to head for Ember Hollow" / "Roland proposes to retreat and heal" |
| Party decision | 2 | "The party decides to travel toward [destination]." |
| Travel departure | 2–3 | Route description with terrain and estimated hours |
| Encounter | 2 | "The party encounters N [enemy name]s!" |
| Combat actions | 3 | Per-hero skill usage: "Roland uses Power Strike." |
| Combat effects | 3 | "The enemy staggers from the blow." / "A critical hit!" |
| Combat outcome | 3 | "The party defeats the [enemy]. N enemies slain, X XP earned." |
| Arrival | 1 | "Day N — The party arrives at [location]." |
| Town rest | 2–3 | Healing and rest messages |
| Camping | 2–3 | Camp setup, watch volunteer |
| Night ambush | 2–3 | Ambush detection, combat, and outcome |
| Day summary | 1 | "Day N ends. N encounters fought." |
| Journey end | 1–3 | Final outcome messages |

### Hero Personality in Decisions

Each hero's voting preference is influenced by their character traits:

- **Cautious heroes** (`rc > 0`): May propose retreating to a town to heal.
- **Offensive heroes** (`od < 0`): Propose pushing through dangerous terrain.
- **Other heroes**: Propose heading for the next destination on the route.

This makes each hero's personality visible in the narrative, creating emergent stories where a reckless rogue might drag the party into danger while a cautious paladin argues for the safe route.
...
| Loading | No change |
| Battle Screen | Add scrollable narrative log panel with 3-level verbosity toggle (Headlines/Standard/Detailed). Day-by-day playback (1 day/sec). Log auto-scrolls as new entries appear for the current day. Only shown in story mode. |
| Results Screen | Add narrative log viewer with verbosity toggle and expandable full log. Show story-specific KPIs (locations visited, towns rested, ambushes survived). Only shown in story mode. |

The verbosity toggle lets the player choose how much detail to see:
- **Headlines** (Level 1): Only major events — day starts, arrivals, journey end.
- **Standard** (Level 2): Includes hero proposals, party decisions, encounters, town/camp events.
- **Detailed** (Level 3): Includes all combat actions per hero, effects, terrain descriptions.
