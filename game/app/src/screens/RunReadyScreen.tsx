...
import { CrossedSwordsIcon, PlayIcon } from '../components/icons';
...
export function RunReadyScreen({ onPlay }: RunReadyScreenProps) {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-stone-900 text-stone-100 gap-6 px-6 text-center">
      <CrossedSwordsIcon size={64} className="text-amber-500"/>
      ...
      >
        <span className="inline-flex items-center gap-2"><PlayIcon size={20}/> Play</span>
      </button>
    </div>
  );
}
