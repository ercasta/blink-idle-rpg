# Blink Idle RPG - Engine Examples
...
They are **engine test programs**, not game demos.  The actual game is in
`game/brl/` and is deployed as a React web app to GitHub Pages.
...
```bash
npm run test:harness   # compile + run all engine example tests (requires Rust)
```
